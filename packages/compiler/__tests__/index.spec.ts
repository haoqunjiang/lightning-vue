...
import { runSharedCssModulesCompileTests } from "./compileStyle.shared";
import { createCompilerRequire } from "../src/nodeRequire";

runSharedCssModulesCompileTests("@lightning-vue/compiler", compiler.compileStyleAsync);

describe("@lightning-vue/compiler", () => {
  ...
  test("compileStyle and compileStyleWithLightningCss are the same engine", () => {
    ...
    expect(compiler.compileStyle(options)).toEqual(compiler.compileStyleWithLightningCss(options));
  });
