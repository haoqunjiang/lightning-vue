...
import type { SelectorParserOptions } from "@lightning-vue/utils";
import type { Selector, SelectorComponent, SelectorList, TokenOrValue } from "lightningcss";

export type ScopeCarrierKind = "deep" | "global" | "slotted";

export type ScopeCarrierSelector = Extract<SelectorComponent, { type: "pseudo-class" }> & {
...
const scopeCarrierNames = new Set<ScopeCarrierKind>(["deep", "global", "slotted"]);
...
export function preludeIsPureGlobalCarrier(prelude: string): boolean {
  try {
    const selectors = parseSelectorListFromString(prelude, scopeCarrierParserOptions);
