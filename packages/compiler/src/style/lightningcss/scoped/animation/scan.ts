export function findTopLevelColon(segment: string): number {
  return findTopLevelSeparatorIndex(segment, ":");
}

export function findTopLevelSeparatorIndex(source: string, separator: string): number {
  let bracketDepth = 0;
  let parenDepth = 0;
  let quote: '"' | "'" | undefined;
