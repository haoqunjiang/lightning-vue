...
const quotedKeyframesPreludeRE = /^(@(?:-\w+-)?keyframes\b\s+)(['"])((?:\\.|(?!\2)[\s\S])*)\2/i;
...
export function rewriteKeyframesPreludes(
  ...
  walkCssBlockPreludes(source, (prelude) => {
    ...
    if (quotedMatch) {
      const rewrittenName = rewriteRawAnimationIdentifier(quotedMatch[3], keyframes);
      if (!rewrittenName || rewrittenName === quotedMatch[3]) {
        return;
      }

      const contentStart = prelude.start + leadingWhitespaceLength + quotedMatch[1].length + 1;
      s.overwrite(contentStart, contentStart + quotedMatch[3].length, rewrittenName);
      changed = true;
      return;
    }
    ...
    const rewrittenName = rewriteRawAnimationIdentifier(unquotedMatch[2], keyframes);
    if (!rewrittenName || rewrittenName === unquotedMatch[2]) {
      return;
    }

    const contentStart = prelude.start + leadingWhitespaceLength + unquotedMatch[1].length;
    s.overwrite(contentStart, contentStart + unquotedMatch[2].length, rewrittenName);
    changed = true;
  });
