...
export function rewriteAnimationDeclarationsInBlock(
  ...
  for (const child of block.children) {
    changed =
      rewriteAnimationDeclarationsInRange(source, s, cursor, child.start, keyframes) || changed;
    changed = rewriteAnimationDeclarationsInBlock(child, source, s, keyframes) || changed;
    cursor = child.end;
  }

  return (
    rewriteAnimationDeclarationsInRange(source, s, cursor, block.bodyEnd, keyframes) || changed
  );
}
...
function rewriteAnimationDeclarationsInRange(
  ...
  for (let index = start; index < end; index++) {
    ...
    changed = rewriteAnimationDeclaration(source, s, declarationStart, index, keyframes) || changed;
    declarationStart = index + 1;
  }

  changed = rewriteAnimationDeclaration(source, s, declarationStart, end, keyframes) || changed;
...
function rewriteAnimationDeclaration(
  ...
  if (!animationPropertyRE.test(property) && !animationNamePropertyRE.test(property)) {
    return false;
  }
