...
export function isDeclarationOnlyAtRuleSubtree(block: CssBlockNode, source: string): boolean {
  ...
  if (!atRuleCanPropagateDeclarationWrapper(block.normalizedPrelude)) {
    return block.children.every((child) => isDeclarationOnlyAtRuleSubtree(child, source));
  }
