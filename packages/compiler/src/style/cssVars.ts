...
export function genVarName(id: string, raw: string, isProd: boolean): string {
  if (isProd) {
    return hash(id + raw).replace(/^\d/, (r: string) => `v${r}`);
  }

  return `${id}-${getEscapedCssVarName(raw, false)}`;
}

export function genCssVarReference(id: string, raw: string, isProd: boolean): string {
...
export function rewriteCssVarsInStyleSource(source: string, id: string, isProd: boolean): string {
...
export function rewriteCssVarsInStyleSourceWithMap(
  ...
  while ((match = vBindRE.exec(maskedSource))) {
    ...
    s.overwrite(match.index, end + 1, genCssVarReference(id, source.slice(start, end), isProd));
    changed = true;
  }
