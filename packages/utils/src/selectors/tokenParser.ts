...
export class TokenSelectorParser extends SelectorParserBase<Token["type"]> {
  ...
  protected readAttributeNameWithNamespace(): {
    ...
    if (afterIdentifier && afterIdentifier.type === "delim" && afterIdentifier.value === "|") {
      this.index++;
      return {
        name: this.readIdentifier(),
        namespace: {
  ...
  protected isAtSelectorListBoundary(endType?: Token["type"]): boolean {
    ...
    return tokenType == null || tokenType === "comma" || (!!endType && tokenType === endType);
  }
  ...
  protected recordParsedSelectorSource(_selector: Selector, _marker: void): void {}
}
