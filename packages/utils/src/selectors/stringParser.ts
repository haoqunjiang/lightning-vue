...
import { decodeCssEscape, isIdentifierContinue, isIdentifierStart } from "./identifiers";
...
export class StringSelectorParser extends SelectorParserBase<string, number> {
  ...
  protected parseComponents(): SelectorComponent[] {
    ...
    if (current === "*") {
      if (this.peek(1) === "|") {
        this.index += 2;
        return [{ type: "namespace", kind: "any" }, this.parseNamespacedTarget()];
      }
      this.index++;
    ...
    if (current === "|") {
      this.index++;
      return [{ type: "namespace", kind: "none" }, this.parseNamespacedTarget()];
    }

    if (current === "&" && this.peek(1) === "|") {
      this.index += 2;
      return [{ type: "namespace", kind: "named", prefix: "&" }, this.parseNamespacedTarget()];
    }

    if (!isIdentifierStart(current) && current !== "\\") {
      throw new Error(`Unsupported selector token: "${String(current)}".`);
    }
  ...
  protected parseCombinator(): Extract<SelectorComponent, { type: "combinator" }> | undefined {
  ...
  protected readAttributeCaseSensitivity(): ParsedCaseSensitivity | undefined {
  ...
  protected readIdentifier(): string {
    ...
    while (!this.isDone()) {
      ...
      if (
        current != null &&
        (value === "" ? isIdentifierStart(current) : isIdentifierContinue(current))
      ) {
        value += current;
        this.index++;
        continue;
  ...
  protected consumeComment(): boolean {
    if (this.sourceContainsComments && this.peek() === "/" && this.peek(1) === "*") {
  ...
  protected recordParsedSelectorSource(
    ...
    const rawSelectorSource = this.source.slice(selectorLeadingTriviaStart, this.index).trim();
