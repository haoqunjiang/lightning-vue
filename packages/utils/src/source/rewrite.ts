...
export type CssSourceMapMerge<TMap extends object> = (currentMap: TMap, nextMap: object) => TMap;
...
export function rewriteCssSelectorSource(
  ...
  const { tryRewritePreludeDirect, parserOptions, appendRewrittenSelectors } = options;
  const output: string[] = [];
  let chunkStart = 0;
...
function rewriteSelectorPrelude(
  prelude: string,
  tryRewritePreludeDirect: ((prelude: string) => string | undefined) | undefined,
