...
import { parseSelectorListFromString, stringifySelector } from "../src/selectors";
 ...
 *
 * This suite covers the selector-facing API of `@lightning-vue/utils`.
 *
 * Not migrated:
...
function parseAndStringify(input: string, options?: SelectorParserOptions): string {
...
describe("adapted from lossy.mjs", () => {
  ...
  runSupportedCases([
    ...[
      ...
      ["namespace - inside attribute (2), space before", " [  postcss|href]", "[postcss|href]"],
      ["namespace - inside attribute (2), space after", "[postcss|href ] ", "[postcss|href]"],
      ["namespace - inside attribute (3), space before", " [  *|href=test]", "[*|href=test]"],
      ["namespace - inside attribute (3), space after", "[*|href=  test  ] ", "[*|href=test]"],
      ["namespace - inside attribute (4), space after", "[|href=  test  ] ", "[|href=test]"],
...
describe("stringifier regressions surfaced by the upstream migration", () => {
  test("custom selector-list functions round-trip their parsed selectors", () => {
    expect(parseAndStringify(":current(p, li, dt, dd)", customSelectorListFunctionOptions)).toBe(
      ":current(p, li, dt, dd)",
    );
  });
  ...
  test("standard pseudo-class functions round-trip", () => {
    ...
    expect(parseAndStringify(":nth-last-of-type(3n-2)")).toBe(":nth-last-of-type(3n-2)");
